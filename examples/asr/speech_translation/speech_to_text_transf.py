...
from nemo.collections.asr.models import EncDecTransfModelBPE, EncDecTransfModelBPEMerged
from nemo.core.config import hydra_runner
from nemo.utils import logging
from nemo.utils.exp_manager import exp_manager


@hydra_runner(config_path="../conf/transformer_decoder/", config_name="speech_translation_with_tts")
def main(cfg):
    logging.info(f'Hydra config: {OmegaConf.to_yaml(cfg)}')

    trainer = pl.Trainer(**cfg.trainer)
    exp_manager(trainer, cfg.get("exp_manager", None))
    asr_model = EncDecTransfModelBPEMerged(cfg=cfg.model, trainer=trainer)

    # Initialize the weights of the model from another model, if provided via config
    asr_model.maybe_init_from_pretrained_checkpoint(cfg)

    enc_weights = OrderedDict()
    weights = torch.load("/workspace/models/test/rnnt.ckpt")
