...
__all__ = ['EncDecTransfModelBPEMerged']
...
def pad_and_merge(a, b, pad_id=0):
    if a.shape[-1] == b.shape[-1] or len(a.shape) == 1:
        return torch.cat((a, b), dim=0)
    elif (a.shape[-1] > b.shape[-1]):
        pad_shape = list(b.shape)
        pad_shape[-1] = a.shape[-1] - b.shape[-1]
        pads = torch.full(size=pad_shape, fill_value=pad_id, device=b.device)
        b = torch.cat((b, pads), dim=-1)
        return pad_and_merge(a, b, pad_id)
    else:
        pad_shape = list(a.shape)
        pad_shape[-1] = b.shape[-1] - a.shape[-1]
        pads = torch.full(size=pad_shape, fill_value=pad_id, device=a.device)
        a = torch.cat((a, pads), dim=-1)
        return pad_and_merge(a, b, pad_id)


class EncDecTransfModelBPEMerged(ASRModel, ExportableEncDecModel, ASRBPEMixin):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        self.preprocessor = EncDecTransfModelBPEMerged.from_config_dict(self._cfg.preprocessor)
        self.encoder = EncDecTransfModelBPEMerged.from_config_dict(self._cfg.encoder)
        ...
        # CTC decoder
        self.ctc_decoder = EncDecTransfModelBPEMerged.from_config_dict(self._cfg.ctc_decoder)
        ...
        if hasattr(self._cfg, 'spec_augment') and self._cfg.spec_augment is not None:
            self.spec_augmentation = EncDecTransfModelBPEMerged.from_config_dict(self._cfg.spec_augment)
        else:
...
#         )
    ...
    def setup_training_data(self, train_data_config: Optional[DictConfig]):
        ...
        if self.use_text_data:
            ...
            # Need to set this because if using an IterableDataset, the length of the dataloader is the total
            # number of samples rather than the number of batches, and this messes up the tqdm progress bar.
    ...
    def preprocess_audio_text_batch(self, batch):
        if self.use_text_data:
            if self.use_audio_data:
        ...
        if text_batch is not None:
            ...
            for i in range(len(text_batch)):
                if text_batch[i].ndim == 3:
                    # Dataset returns already batched data and the first
                    # dimension of size 1 added by DataLoader is excess.
                    text_batch[i] = text_batch[i].squeeze(dim=0)
            src_ids, src_mask, transcript, tgt_mask, _ = text_batch
            with torch.no_grad():
        ...
        if audio_batch is not None:
            ...
            audio_signal, audio_signal_len, audio_transcript, audio_transcript_len = audio_batch
            audio_signal, audio_signal_len = self.preprocessor(
                input_signal=audio_signal, length=audio_signal_len
            )
            if text_batch is not None:
                signal = pad_and_merge(audio_signal, signal, self.preprocessor.featurizer.pad_to)
                signal_len = pad_and_merge(audio_signal_len, signal_len)
                transcript = pad_and_merge(audio_transcript, transcript, self.tokenizer.pad_id)
                transcript_len = pad_and_merge(audio_transcript_len, transcript_len)
            else:
                signal, signal_len, transcript, transcript_len = \
                    audio_signal, audio_signal_len, audio_transcript, audio_transcript_len
    ...
    def training_step(self, batch, batch_nb):
        ...
        signal, signal_len, transcript, transcript_len = self.preprocess_audio_text_batch(batch)
