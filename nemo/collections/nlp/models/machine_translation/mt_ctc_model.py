...
class MTCTCModel(EncDecNLPModel):
    """
    ...
    def __init__(self, cfg: MTEncDecModelConfig, trainer: Trainer = None):
        ...
        self.loss_fn = CTCLoss(num_classes=self.decoder_tokenizer.unk_id, zero_infinity=True)
    ...
    def forward(self, src, src_mask, tgt, tgt_mask):
        ...
        src = src.view(-1, 1).repeat(1, 2).view(batch_size, seq_len*2)
        src_mask = src_mask.view(-1, 1).repeat(1, 2).view(batch_size, seq_len*2)
        src_hiddens = self.encoder(src, src_mask)
        log_probs = self.log_softmax(hidden_states=src_hiddens)
        beam_results = torch.max(log_probs, dim=-1)[1]
    ...
    def training_step(self, batch, batch_idx):
        ...
        src_lengths = src_mask.sum(dim=-1) * 2
    ...
    def eval_step(self, batch, batch_idx, mode):
        ...
        eval_loss = self.loss_fn(
            ...
            targets=tgt_ids,
            input_lengths=src_lengths,
            target_lengths=tgt_lengths).cpu().numpy()
        ...
        for tr in beam_results.cpu().numpy():
            ...
            for i in range(1, len(tr)):
                if tr[i] != current_char:
                    trans.append(tr[i])
                    current_char = tr[i]
                if tr[i] == self.decoder_tokenizer.eos_id:
                    break
            translations.append(self.decoder_tokenizer.ids_to_text(trans))
